...
(defn valid-game [game]
  (let [game-maxes (update-vals (parse-game game) (partial apply max))]
    (not (reduce #(or % %2) (map neg? (vals (merge-with - bag-conts game-maxes)))))))
(defn soln-p1 [data]
  (reduce + (map #(Integer/parseInt (last (re-find #"(?:Game)\s(\d+)" %)))
                 (map first (filter #(valid-game (last %)) data)))))

;; part 2
(defn soln-p2 [data]
  (reduce + (map #(reduce * (vals %)) (map #(update-vals (parse-game %) (partial apply max)) (map last data)))))

(defn -main
  "Evaluate solutions"
  [& args]
  (let [data (map #(str/split % #": ") (str/split-lines (slurp "data/day2.txt")))]
